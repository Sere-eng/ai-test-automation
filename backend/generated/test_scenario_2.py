...
def do_login_and_go_to_laboratory(page: Page) -> None:
    ...
    if not lab_url or not lab_user or not lab_password:
        raise RuntimeError(
            "LAB_URL/LAB_USERNAME/LAB_PASSWORD must be set in environment"
        )
    ...
    page.locator(".eng-app-viewport-spinner-container").wait_for(
    ...
    page.get_by_role("option").filter(
        has_text=re.compile(r"organizzazione.*sistema", re.I)
    ).click()
...
def test_scenario_2(page: Page):
    """Scenario: Accesso tramite contatori in dashboard (scenario_2)"""
    ...
    page.get_by_role("button", name="Laboratorio").click()
    # wait_for_load_state
    page.wait_for_load_state("domcontentloaded")
    # click_smart (strategy=role)
    page.get_by_role("button", name="Modifica").click()
    # click_smart (strategy=role)
    page.get_by_role("button", name="Aggiungi filtro").click()
    # fill_smart (strategy=label)
    page.get_by_label("Nome filtro*").fill("Filtro Test")
    # click_smart (strategy=role)
    page.get_by_role("button", name="Conferma").click()
    # click_smart (strategy=text)
    page.get_by_text("68").first.click(force=True)
    # wait_for_load_state
    page.wait_for_load_state("domcontentloaded")
