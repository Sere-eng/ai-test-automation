...
class TestAgentMCP:
    ...
    def _build_system_message(self):
        ...
        prompt_template = (
            self.custom_prompt if self.custom_prompt else get_lab_optimized_prompt()
        )
    ...
    async def run_test_async(self, test_description: str, verbose: bool = True) -> dict:
        ...
        artifacts: list[dict] = []
        final_answer: str = ""
        pending_inputs: dict = {}  # tool_name -> input, per allegare args agli step
        ...
        async for ev in self.agent.astream_events(
            ...
            if event_type == "on_tool_start":
                ...
                if tool_name and inp is not None:
                    pending_inputs[tool_name] = inp
            if verbose and event_type == "on_tool_end":
                print(f"[on_tool_end] {tool_name}")
                out_raw = ev.get("data", {}).get("output")
                if out_raw is not None:
                    out_clean = parse_tool_output(
                        out_raw
                    )  # stesso output degli step: niente repr/escape
            ...
            if event_type == "on_tool_end":
                ...
                step = step_from_tool_end(tool_name, output_obj)
                step["input"] = pending_inputs.pop(tool_name, {})
                steps.append(step)
        ...
        # Summary deterministica a partire dalla trace MCP (utile anche per i test custom)
        trace_summary = None
        try:
            trace = extract_trace(steps)
            if trace:
                trace_summary = summarize_trace(
                    trace, scenario_id="mcp_run", scenario_name=None
                )
        except Exception:
            trace_summary = None
        ...
        if verbose:
            ...
            # Filtra output "tool_call" legacy (es. <function=capture_screenshot>...)
            printable_notes = None
            if isinstance(final_answer, str):
                stripped = final_answer.strip()
                # Se è solo un blocco di tool_call XML-like, non stamparlo
                if stripped and not stripped.startswith("<function="):
                    printable_notes = final_answer

            if printable_notes:
                print("\nNOTES (model text, not used for pass/fail):")
                print(printable_notes)

            if trace_summary:
                print("\nTRACE SUMMARY (from MCP tool trace):")
                print(trace_summary)
            print(f"{'='*80}\n")
